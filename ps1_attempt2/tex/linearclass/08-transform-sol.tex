%\input{../macros.tex}
%\begin{document}
\begin{answer}
	Transforming the 2nd x variable by taking the log we see that the resultant binary classification problem looks as lot better fit by the GDA assumptions.
	\begin{figure}[H]
	\includegraphics{C:/Users/benja/coding/cs229/ps1_attempt2/src/linearclass/gda_pred1_logx2_fig.png}
	\end{figure}

The corresponding decision boundary on the pre-transformed data is shown here.

	\begin{figure}[H]
	\includegraphics{C:/Users/benja/coding/cs229/ps1_attempt2/src/linearclass/gda_pred_1_logtrans_fig.png}
\end{figure}
\end{answer}
%\end{document}